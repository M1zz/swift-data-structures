 ...
 **Pivot** 을 기준으로 작은 것은 왼쪽, 큰 것은 오른쪽으로 분할 정복합니다.
 ...
 - **최악 케이스**: 이미 sorted array에 첫 원소 pivot → O(n²)

 ## Merge Sort

 항상 절반씩 나누어 정렬 후 병합합니다.
 **stable sort(Stable Sort)** 이 필요하거나 LinkedList 정렬에 적합합니다.
 ...
 - **stable sort 필요 시**: `sort(by:)` — Swift의 기본 sort는 stable sort 보장(SE-0372)
...
print("\n── sorted array \(n)개 (Quick Sort 최악 케이스 주의) ──")
measureTime("Swift sort()") { var a = sortedArr; a.sort() }
measureTime("Merge Sort  ") { _ = mergeSort(sortedArr) }

/*:
 > **Observation Points**
 > - random array: Quick Sort가 빠른지 확인
 > - sorted array: Quick Sort(last pivot)가 느려지는지 확인
