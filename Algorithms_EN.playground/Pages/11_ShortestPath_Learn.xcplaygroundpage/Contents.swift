...
/*:
 ...
 | 알고리즘 | 시간 | negative weights | negative cycle | 용도 |
 |----------|------|------------|------------|------|
 ...
 **탐욕적(Greedy)** 알고리즘입니다. Min Heap으로 가장 가까운 미방문 노드를 선택합니다.
 ...
 모든 간선을 **V-1번** Relax합니다. V번째도 갱신된다면 negative cycle이 존재합니다.
 느리지만 negative weights, 음수 cycle detection 모두 가능합니다.
...
func bellmanFord(n: Int, edges: [(u: Int, v: Int, w: Int)], start: Int) -> [Int]? {
    ...
    // 음수 cycle detection
    for e in edges {
        if dist[e.u] + e.w < dist[e.v] { return nil }  // negative cycle 존재
    }
    return dist
}
