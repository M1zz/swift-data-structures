...
/*:
 ...
 Graph는 **Vertex**과 **Edge**으로 이루어진 자료구조입니다.
 표현 방법은 크게 두 가지:

 **그래프 표현 방식 비교**
 - **인접 행렬**: 공간 O(V²) / 간선 확인 O(1) / 이웃 순회 O(V)
 - **인접 리스트**: 공간 O(V+E) / 간선 확인 O(degree) / 이웃 순회 O(degree)

 > In practice는 대부분 **인접 리스트**를 사용합니다 (희소 그래프가 대부분).

 ## BFS vs DFS

 **BFS vs DFS 비교**
 - **BFS**: Queue 사용 — 최단 경로 (가중치 없음), 레벨별 탐색
 - **DFS**: Stack/재귀 사용 — 사이클 탐지, 위상 정렬, 경로 탐색
